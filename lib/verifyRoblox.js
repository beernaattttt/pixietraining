...
/**
 * Verifies a request came from the Discord bot acting on a member's behalf
 * (e.g. clicking "Join" on a training embed). Deliberately a SEPARATE
 * identity and secret from verifyRobloxServer — the bot and the Roblox game
 * server are different trust boundaries with different blast radii if
 * compromised, so neither secret should be able to impersonate the other.
 */
export function verifyDiscordBotServer(req) {
  const provided = req.headers.get("x-discord-bot-server-key") || "";
  const expected = process.env.DISCORD_BOT_SERVER_KEY || "";
